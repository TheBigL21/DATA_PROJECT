...
def main() -> int:
    ...
    print(f"Destination: {REPO_ROOT.as_posix()}\n")
    ...
    # Run verification
    try:
        from src.verify_runtime_files import main as verify_main

        print("Running verification...\n")
        return int(verify_main())
    except Exception as e:
        print(f"WARNING: Could not run verification script automatically: {e}")
        print("You can run it manually with: python -m src.verify_runtime_files")
        return 0
