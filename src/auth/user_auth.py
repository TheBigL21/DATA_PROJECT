...
class UserAuth:
    ...
    def __init__(self, users_db_path: str = None):
        """
        Initialize authentication system

        Args:
            users_db_path: Path to users database JSON file (defaults to data/raw/users.json)
        """
        if users_db_path is None:
            # Default to data/raw, compute repo root
            repo_root = Path(__file__).parent.parent.parent
            users_db_path = str(repo_root / "data" / "raw" / "users.json")
